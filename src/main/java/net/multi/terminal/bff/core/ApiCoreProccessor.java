...
public class ApiCoreProccessor {
    ...
    public Void process(ApiIdentity identity, ChannelHandlerContext ctx, FullHttpRequest httpRequest) throws Exception {
        ...
        try {
            ...
            commonMsgReq = apiCodec.decode(inputMsg);
            // 获取报文转换器
            MsgSerializer serializer = context.getSerializer();
            // 反序列化请求
            ApiReq req = serializer.deserialize(identity, commonMsgReq);
            // 注入session对象
            ...
            log.error("API执行失败", e);
            exceptionHandle(e, identity, commonMsgReq);
        }
        return null;

    }

    public void exceptionHandle(Throwable throwable, ApiIdentity identity, CommonMsg msg) throws ApiException {
        // 序列化
        try {
